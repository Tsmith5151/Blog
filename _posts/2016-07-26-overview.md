...
**Random Forest:** An adaptation of decision trees. Intuitively, a random forest can be considered as a collection or an ensemble of decision trees. This approach greatly reduces the variances and bias of the estimates, especially for unbalanced data. Typically, as the larger the number of trees increase, then the better the performance of the classifier will be, however this can result in an increased computational cost. The following steps for the alogirthm are:

	1. Take the set of training examples and bootstrap samples into k subsets (in other words randomly choose k samples from the training set with replacement).
	2. Grow a full decision tree from the bootstrap sample (with no pruning). At each node: Randomly select d features without replacement. (as a result, you will end up with k different trees, each tree looks at different attributes in the data)
	3. Split the node using the feature that provides the best split according to the objective function, for instance, by maximizing the information gain.
	4. At prediction time, you take the new data point and classify it using all of the k trees and then use the majority vote (i.e. 8 out of the 10 trees classified that John will play). 

***Strength***
One big advantage of random forests is that we don't have to worry so much about choosing good values of the hyperparameters. Also, we typically don't need to prune the random forest since the ensemble model is quite robust to noise from the individual decision trees. 
