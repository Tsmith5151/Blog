...
The Boston housing market is highly competitive and the goal is to be one of the best real estate agents in the area. To compete in the real estate market, a few basic machine learning concepts were applied to assist a client with finding the best selling price for their home. The [Boston Housing](http://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_boston.html) dataset contains aggregated data on various features for houses in Greater Boston communities, including the median value of homes for each of those areas. The supervised machine learning algorithm I will be working with is Decision Trees to predict the best selling price for the client's home. In this case, the target is the median home price (MEDV), which is a continous variable, thus decision trees can be applied to regression problems as in this case. Again, this problem is an example of supervised learning. We have a dataset that consists of a list of housing features and the goal is to predict the label (MEDV); note that the labels supervises the learning. Supervised learning is also known as "Function Approximation", meaning that given a bunch of x,y pairs and the goal is to find some function (f) that will map some new x to a proper y.  

The source code developed for this project can be found in the ipython notebook boston_housing.ipynb and the complete repo can be found [here](https://github.com/Tsmith5151/Boston-Housing-Prices). The source code is discussed throughout this Blog, but keep in mind that not every output generated by the code is shown below; please refer to the repo link. Let's get started.

### Python Libraries:
First, a list of the python packages that were utilized in the work can be found in the ipython notebook. Mostly will be working with pandas DataFrames and for visualizations/plots I will be using matplotlib and seaborn. Furthermore, I will be calling the Machine Learning classifiers and data mining tools from [Scikit-Learn](http://scikit-learn.org).
...
If we were only given a plain CSV file, we can work with this as well. For example in Pandas we can call the read_csv function as shown below:
...
As mentioned above, the Boston Housing dataset it is already in the pandas data structure via the example datasets available from Scikit-Learn. Some manipulation will be necessary to identify the target and feature columns of the DataFrame. 
...
**Cost Function**:
So when estimating the function f(x) given the data to model, search all possible values of w0 and w1 such that the results is one that minimizes the cost function. In Linear Regression, Gradient descent can be implemented, which is an iterative approach for minimizing this cost function. The algoirhtm begins with an initial guess of the solution and then the gradient of the function at that point is computed. The step the solution in the negative direction of the gradient and we repeat the process. The gradient descent algorithm will eventually converge in which the gradient is zero (i.e. the local minimum at point D). The goal is to determine the values of w0 and w1 such that the loss function, the least squared error, is minimized. A visualization is shown below:

<img src = "https://tsmith5151.github.io/Blog/img/Boston/gradient_plot.png">

*Source: CS2299 Lecture Notes on Linear Regresstion*

Now, we can implement gradient descent with Linear Regression to the dataset to find the best relationship between X & Y. The relationship between Crime (X) and the MEDV (Y) is explored below:
...
As shown below, the case of max depth = 1, the training error begins to significanlty increase as the training size increases and likewise, the error for the test error remains relatively high. Thus this situation where the max depth of the decision tree is 1, the model would be underfitting as the learning value is restricted to one level of the decision tree and does not allow the training set to learn data adequately. A low complexity decision tree results in high bias. For a max depth = 10, this would clearly illustrate a high variance and overfitting the data. The model has virtually memorized the training data but will not be expected to perform well with out-of-sample
data.
...
#### Fit Model:
Before we get to the actual code, first lets discuss **GridSearch** as this hpyerparameter optimization technique will be implemented. The performance of the model depends on the hyperparameters provided to the algorithm when training the model. It is desirable to find the right combination of the hyperparameters during training. [sklearn.grid_search.GridSearchCV](http://scikit-learn.org/stable/modules/generated/sklearn.grid_search.GridSearchCV.html#sklearn.grid_search.GridSearchCV) is
an exhaustive search that trains and evaluates a model for all possible combination of hyperparameters that produce the best model. In this case, the max depth: 1-10 is evaluated to determine how complex the tree should be based on the performance metric: metric mean squared error.

Furthermore, lets briefly discuss **Cross-Validation** and how it is performed. A common occurrence from machine learning algorithms is a degree of bias that can exist when random sampling the data and splitting the data between training and testing. To avoid sampling issues, which can cause the training set to be
too optimistic, cross-validation is a statistical approach that computes the average on multiple test sets. One of the most common iterators that performs the k-fold cross-validation is `KFolds` and the steps are as follows:
...
The `train_test_split()` method can be very simple to perform however the downside is the training data is reduced by 30% when finding the best regression method. Using the default GridSearchCV parameters, the cross-validation generator is set to 3-fold CV. So for instance, the housing dataset consists of
506 observations per feature and if k=3, 169 observations would be in each fold. The initial iteration entails the first fold containing the testing data (i.e. 169 observations) and the remaining folds (2-3) containing 337 observations, 168 each. In second iteration, the testing set is now fold #2 while fold #1 and fold #3 are the training set. We train K different models, with each time leaving out a single subset for measuring the cross-validation error. The final cross-validation error is calculated by taking the mean or median of the K models. The advantage of cross-validation provides a more accurate estimate of the out-of-sample accuracy and is more efficient as every observation is used for both training and testing as oppose to the train/test/split method.
...
As the max depth of the tree is increased, the training error begins to exponentially decline and approaches zero as the model becomes more complex. The testing error begins to differs from the training error near an max_depth = 4, which best generalizes the dataset. At the instance where the model begins to behave too much like the training data then it might be observed that overfitting is occurring as the model does not perform well when testing and the error rates will begin to significantly differ. A decrease in the training error implies that the model is becoming better at fitting the data; when the testing error plateaus and is no longer decreasing, additional knowledge is not gained on the out-of-sample data. If the error does not reduce any further during testing, then the complexity is increased for no reason and therefore overfitting occurs.

<img src = "https://tsmith5151.github.io/Blog/img/Boston/Model.Complexity.png">


Therefore, based on the model complexity graph as shown above, the model that best generalizes the data
is when max_depth = 4. Calling the `grid.best_params_` from GridSearchCV confirms the maximum level to split the decision tree is 4.
...
=3) were performed by using GridSearch to determine the max depth of the decision tree and the predicted housing price. Both results validate that the max_depth = 4 and the suggested value of the home is
